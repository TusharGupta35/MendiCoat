/**
 * The live half of Mendi Coat.
 *
 * Everything particular to this game lives here — its rooms, its series, its
 * bots, and every socket event it answers. The socket server owns none of it:
 * it creates the connection and offers each game module a chance to listen, so
 * a new game is a new folder rather than another branch in a shared handler.
 */

import { randomUUID } from 'node:crypto';
import type { Server } from 'socket.io';
import { chooseBotCard } from '@/games/mendi-coat/bot';
import { createInitialGameState, validateMove, applyMove } from '@/games/mendi-coat/engine';
...
import { closeMatch, openMatch, trickEntry, type TrickLogEntry } from '@/games/mendi-coat/match-store';
...
/**
 * Forgets a room outright, so a code handed out again later cannot inherit
 * these seats and this half-played match. A no-op for a code this game never
 * held — a deletion is offered to every game module, and only one has it.
 */
export function closeMendiCoatRoom(code: string) {
  rooms.delete(code);
}

export function registerMendiCoatHandlers(io: Server) {
