...
describe('chooseBotCard', () => {
  it('always returns a legal card', () => {
    ...
    expect(chooseBotCard(state, 1, 'normal')?.code).toBe('3S');
  });
  ...
  it('gives a 10 to a partner who is winning the trick', () => {
    ...
    expect(chooseBotCard(state, 2, 'normal')?.code).toBe('10S');
  });
  ...
  it('never throws a 10 into a trick an opponent is winning', () => {
    ...
    const choice = chooseBotCard(state, 2, 'normal');
  ...
  it('wins with the cheapest card that does the job', () => {
    ...
    expect(chooseBotCard(state, 1, 'normal')?.code).toBe('QS');
  });
  ...
  it('spends a 10 to take a trick that already carries one, on hard only', () => {
