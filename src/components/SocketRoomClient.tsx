    ...
    <div className="space-y-4">
      ...
      {gameState ? <section className="space-y-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-4 text-sm text-slate-200"><p>Turn: Seat {gameState.currentTurn + 1}</p><p>Trump: {gameState.trumpSuit}</p><p>Trick: {gameState.trickNumber}</p></div>
        </div>
        <div className="overflow-hidden rounded-2xl border-4 border-amber-950/80 bg-emerald-800 p-2 shadow-[inset_0_0_50px_rgba(0,0,0,0.35)] sm:border-8 sm:p-6">
          <p className="text-center text-xs font-semibold uppercase tracking-[0.22em] text-emerald-100/75">{gameState.trickCards.length ? "Current trick" : "Last completed trick"}</p>
          <div className="mt-3 grid min-h-[280px] grid-cols-[minmax(58px,1fr)_minmax(94px,1.6fr)_minmax(58px,1fr)] grid-rows-[auto_1fr_auto] gap-1 sm:min-h-[410px] sm:grid-cols-[minmax(74px,1fr)_minmax(130px,2fr)_minmax(74px,1fr)] sm:gap-2">
            {[0, 1, 2, 3].map((tableSeat) => {
              ...
              return <div key={tableSeat} className={`z-10 min-w-0 ${position}`}><div className={`truncate rounded-full px-1.5 py-1 text-center text-[9px] font-semibold sm:px-2 sm:text-[10px] ${gameState.currentTurn === tableSeat && gameState.status === "PLAYING" ? "bg-amber-300 text-emerald-950" : "bg-emerald-950/70 text-emerald-100"}`}>{participant.name} · S{tableSeat + 1}</div>{play ? <div className={`animate-card-play mx-auto mt-2 flex h-[4.5rem] w-12 flex-col justify-between rounded-md bg-stone-50 p-1 shadow-lg sm:h-24 sm:w-16 sm:p-1.5 ${suitColor(play.card.suit)}`}><span className="text-xs font-bold sm:text-sm">{cardLabel(play.card)}</span><span className="self-center text-xl sm:text-2xl">{SUIT_SYMBOL[play.card.suit]}</span><span className="self-end rotate-180 text-xs font-bold sm:text-sm">{cardLabel(play.card)}</span></div> : null}</div>;
            })}
            <div className="col-start-2 row-start-2 flex items-center justify-center"><div className="rounded-full border border-amber-300/30 bg-emerald-950/65 px-2 py-1.5 text-center text-[10px] text-amber-100 sm:px-4 sm:py-2 sm:text-xs">{gameState.lastTrick && !gameState.trickCards.length ? `Seat ${gameState.lastTrick.winner + 1} won the last trick` : "Play a card"}</div></div>
          </div>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">{(["A", "B"] as const).map((team) => <div key={team} className="rounded-lg bg-slate-950/60 p-3"><p className="font-medium text-white">Team {team} · Seats {team === "A" ? "1 & 3" : "2 & 4"}</p><p className="mt-1 text-sm text-amber-300">Captured 10s: {gameState.capturedTens[team]}</p><p className="mt-1 text-xs text-slate-400">♠ {gameState.capturedTensBySuit[team].SPADES} · ♥ {gameState.capturedTensBySuit[team].HEARTS} · ♣ {gameState.capturedTensBySuit[team].CLUBS} · ♦ {gameState.capturedTensBySuit[team].DIAMONDS}</p></div>)}</div>
        ...
        <div className="rounded-lg bg-slate-950/60 p-3 sm:p-4"><p className="text-sm text-slate-400">Your hand {seat === null ? "" : `(Seat ${seat + 1})`}</p><div className="mt-2 flex items-end overflow-x-auto px-4 pb-3 pt-8">{player?.cards.map((card, index) => <button key={card.code} type="button" disabled={seat !== gameState.currentTurn} onClick={() => playCard(card)} style={{ marginLeft: index === 0 ? 0 : "-2.15rem" }} className={`relative flex h-24 w-[4.5rem] shrink-0 flex-col justify-between rounded-lg border-2 border-stone-300 bg-stone-50 p-2 font-mono text-sm font-bold shadow-md transition hover:z-20 hover:-translate-y-5 hover:shadow-xl sm:h-28 sm:w-20 sm:text-base disabled:cursor-not-allowed disabled:opacity-55 ${suitColor(card.suit)}`}><span>{cardLabel(card)}</span><span className="self-center text-2xl sm:text-3xl">{SUIT_SYMBOL[card.suit]}</span><span className="self-end rotate-180">{cardLabel(card)}</span></button>)}</div></div>
      </section> : null}
